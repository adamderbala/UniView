...
fn mainloop(appl: &mut window::Application, frame_num: u32) {
    ...
    let contours = get_rects(get_contours(grayscale));
    if frame_num >= INIT_FRAMES && contours.len() > 0 {
        draw::draw_contours(&disp, &mut None, area_filter(contours.clone(), RECT_MIN_AREA_CAR));
    }
    if frame_num < INIT_FRAMES {
